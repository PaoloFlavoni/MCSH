...
int main()
{
  ...
  char nomefile[20];             /* Stringa dove viene scritto il nome
  ...
  for(sign_count=0; sign_count<2; sign_count++) {
    /* Apertura del sign_count-esimo file di dati contenente il
       segnale corrispondenti all'orbita che vogliamo studiare con
       l'analisi in frequenza */
    sprintf(nomefile, "segnale_HH_%d.out", sign_count+1);
    in = fopen(nomefile,"r");
