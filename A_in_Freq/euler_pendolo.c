...
/*Sia tbar = 2*pi/sqrt(2), ovvero il periodo della forzante
  dt = tbar/200
  dt_graph = tbar
  2T = tbar*2^11
  npt = 2*T/dt_graph  
...
int main()
{
    ...
    /*
    i_graph = (dt_graph*8) / dt; Questo valore è 200, in base ai parametri definiti a monte del programma
    e come deve essere il ritratto in fase dell'oscillatore armonico è quello atteso(Forma oblunga sulla destra).
    */
    i_graph = dt_graph / dt;
    printf("Scrive Ogni %ld \n\n", i_graph);
    printf("Difatti i_graph*dt è pari al tempo di scansionamento %lf \n\n", i_graph*dt);
    ...
    {   
        ...
        x = x + y * dt ;
        /*
        Devo togliere la periodicità nella variabile
        angolare per l'
